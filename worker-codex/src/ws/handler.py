"""worker-codex WS handler.

Same routing surface as worker-claude and worker-copilot. SDK-agnostic
messages (projects, files, terminal, browse_fs, upload) go through
`worker_shared`. SDK-specific messages route through `SessionManager`,
which owns a `Session` wrapping a Codex thread from `openai-codex-sdk`.

Differences vs worker-copilot's handler:
- `list_models` returns an empty list - openai-codex-sdk does not
  publish a `list_models()` and Codex uses a single canonical model
  per CLI binary version. Hub falls back to its type-default list
  (currently empty for "codex" - add entries in hub's
  _DEFAULT_MODELS_BY_TYPE if you want autocomplete).
"""
...
async def _route(
    ...
    if msg_type == "list_models":
        # openai-codex-sdk does not expose a list_models call: Codex
        # uses one canonical model per CLI binary version. Return an
        # empty list; hub falls back to its type-default (currently
        # nothing for "codex"; extend _DEFAULT_MODELS_BY_TYPE on the
        # hub side if you want the /model picker to suggest entries).
