...
async def scan_and_register(database: Database, projects_root: Path = PROJECTS_ROOT) -> int:
    ...
    async with database.connect() as conn:
        for entry in sorted(projects_root.iterdir()):
            ...
            await conn.execute(
                """
                INSERT INTO projects (path, name, last_seen_at)
                VALUES (?, ?, datetime('now'))
...
def project_dir_for_cwd(cwd: str, projects_root: Path = PROJECTS_ROOT) -> Path:
    ...
    Claude CLI encodes cwd as: replace '/' with '-'. We don't escape '-' in the
    original path because the CLI doesn't either.
    """
    return projects_root / cwd.replace("/", "-")
