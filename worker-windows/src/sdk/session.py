...
class Session:
    ...
    async def _sdk_init(self) -> None:
        ...
        # Build in-process MCP server map from active Windows modules.
        # Using McpSdkServerConfig (type="sdk") bypasses HTTP entirely - no
        # SSL cert issues, no network round-trip. Falls back to writing
        # settings.json for standalone CLI access outside the hub session.
        from src.main import get_active_modules
        mcp_servers: dict[str, Any] = {}
        for module in get_active_modules():
            instance = None
            if hasattr(module, "mcp_server_instance"):
                instance = module.mcp_server_instance()
        ...
        if not mcp_servers:
            # No in-process servers - fall back to settings.json SSE config
            self._configure_mcp_settings()
    ...
    def _configure_mcp_settings(self) -> None:
        """Write active module MCP configs into ~/.claude/settings.json.

        The Claude SDK reads mcpServers from user-level settings.json
        when setting_sources includes "user". Merges module entries into
        the existing mcpServers block so other user configuration is
        preserved.
        """
        from src.main import get_active_modules
        ...
        # Merge module MCP configs into the mcpServers block
        ...
        for module in modules:
            try:
                ...
                if isinstance(config, dict):
                    mcp_servers[module.name] = config
            except Exception as e:
                logger.warning(
                    "mcp_config_failed",
                    module=module.name,
        ...
        try:
            ...
            logger.info(
                "mcp_settings_written",
                path=str(settings_path),
                modules=[m.name for m in modules],
            )
        except OSError as e:
            logger.error("mcp_settings_write_failed", error=str(e))
    ...
    async def _dispatch(self, msg: Any) -> None:
        ...
        if isinstance(msg, AssistantMessage):
            ...
            # Update our session_id to the actual Claude Code UUID from the
            # ResultMessage. Our internal UUID (generated in __init__) does not
            # match the .jsonl filename Claude Code saves on disk. The frontend
            # needs the real UUID so future resume_session calls pass the right
            # --resume flag to the Claude CLI.
    ...
    def _task_output_path(self, task_id: str) -> Path | None:
        ...
        Cross-platform: uses tempfile.gettempdir() for the temp root and a
        user identifier that works on both Unix and Windows.

        Unix format: /tmp/claude-{uid}/{cwd-slug}/{session_id}/tasks/{task_id}.output
        Windows: %TEMP%/claude-{username}/{cwd-slug}/{session_id}/tasks/{task_id}.output

        cwd-slug is the cwd with every non-alphanumeric character replaced by '-'.
        Returns None if we don't have enough context (no cwd) to build the path.
        """
        if not self._cwd:
            return None
        cwd_slug = re.sub(r"[^A-Za-z0-9]", "-", self._cwd)
        # Unix uses numeric UID, Windows uses username
        try:
    ...
    async def _ensure_credentials(self) -> bool:
        ...
        worker_id = os.environ.get("WORKER_ID", "") or "worker-windows"
        try:
