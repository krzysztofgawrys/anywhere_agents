...
try:
    ...
    # fcntl / pty are Unix-only - fall back to Windows ConPTY implementation.
    try:
        from src.terminal.session import TerminalSession  # type: ignore[assignment]
        _TERMINAL_AVAILABLE = True
    except ImportError:
        TerminalSession = None  # type: ignore[assignment, misc]
...
async def _route(
    ...
    if msg_type == "terminal_open":
        if not _TERMINAL_AVAILABLE:
            await _send_error(send, "not_supported", "Terminal is not available on Windows")
            return terminal
        project_id = payload.get("project_id")
        if not isinstance(project_id, int):
            await _send_error(send, "bad_request", "project_id required")
